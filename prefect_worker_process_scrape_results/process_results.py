...
def extract_data_from_dictobject(raw_dict_object):
    ...
    df = raw_df.select(
        [
            ...
            safe_col(raw_df, "location.coordinates.latitude", pl.Float64).alias(
                "latitude"
            ),
            safe_col(raw_df, "location.coordinates.longitude", pl.Float64).alias(
                "longitude"
            ),
            safe_col(raw_df, "location.address.street.name").alias("street_name"),
            pl.when(safe_col(raw_df, "agency.type") == "agency")
            .then(pl.lit(1))
            .otherwise(pl.lit(0))
            .alias("is_agency_offer"),
            pl.when(pl.col("advertType") == "DEVELOPER_UNIT")
            .then(pl.lit(1))
            .otherwise(pl.lit(0))
            .alias("is_developer_offer"),
            safe_col(raw_df, "target.Security_types", pl.List(pl.Utf8)).alias(
                "Security_types"
            ),
            safe_col(raw_df, "target.Media_types", pl.List(pl.Utf8)).alias(
                "Media_types"
            ),
    ...
    if len(raw_df["featuresByCategory"].to_list()[0]) != 0:
        featuresByCategory_df = pl.from_dict(
            {
                i.get("label"): None if i.get("values") == "" else [i.get("values")]
                for i in raw_df["featuresByCategory"].explode()
            }
        )
    else:
    ...
    new_df = new_df.cast(
        {
            "price": pl.Float64,
            "property_size_m2": pl.Float64,
            "terrain_area": pl.Float64,
        }
    )
